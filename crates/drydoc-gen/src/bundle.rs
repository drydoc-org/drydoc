...
use super::fs2::{Folder, VirtFile, Entry};
...
impl Bundle {
  ...
  pub fn insert_entry<N: Into<String>, E: Into<Entry>>(&mut self, name: N, entry: E) -> std::io::Result<()> {
    self.folder.insert(name, entry)
  }

  pub fn merge(&mut self, child: Bundle) -> std::io::Result<()> {
    self.manifest.merge(child.manifest).unwrap();
