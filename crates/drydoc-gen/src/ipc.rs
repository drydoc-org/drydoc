...
use crate::actor::{Actor, Addr, Receiver, WeakAddr, map::{Map, Msg as MapMsg}};
use client::RequestData;
use drydoc_model::{Encoding, Message, client, server};
use log::error;
...
trait Responder {
  fn resolve(&self, data: client::ResponseData) -> Result<(), ()>;
}
  ...
  W: 'static + AsyncWrite + Send + Unpin
{
  write: W,
  read: Option<R>,
  outstanding_requests: Addr<MapMsg<u64, Arc<dyn Responder + Send + Sync>>>,
  request_id_iter: u64,
  encoding: Encoding
}
  ...
  W: 'static + AsyncWrite + Send + Unpin
{
  pub fn new(read: R, write: W) -> Self {
    Self {
      read: Some(read),
      write,
      outstanding_requests: Map::new().spawn(),
      request_id_iter: 0,
  ...
  async fn on_request(this: &mut IpcReader<R>, request: client::Request) {
    use client::{Request, OpenRequest, ReleaseRequest};
  ...
  async fn open_context(&mut self, open: OpenContext) {
    use server::{Request, OpenContextRequest};
    ...
    self.outstanding_requests.insert(self.request_id_iter, Arc::new(responder)).await.unwrap();
  ...
  async fn close_context(&mut self, close: CloseContext) {
    use server::{Request, CloseContextRequest};
  ...
  async fn generate(&mut self, generate: Generate) {
    use server::{Request, GenerateRequest};
    
    let Generate { context_id, params, path, res } = generate;
...
impl Addr<IpcMsg> {
  ...
  pub async fn generate(&self, context_id: u32, params: HashMap<String, String>, path: String) -> Result<client::GenerateResponse, Error> {
    let (tx, rx) = channel();
    self.send(Generate {
      context_id,
      params,
...
use tokio::{
  process::{ChildStdout, ChildStdin},
  net::TcpStream
};
